...
class App extends Component {
  ...
  deleteItem = (ind) => {
    let item = [...this.state.invoiceItems]; //pure function
  ...
  convertPdf = () => {
    ...
    this.state.invoiceItems.map((item, index) => {
      ...
      doc.text(135, 82 + index * 8, "N " + item.itemRate);
      doc.text(175, 82 + index * 8, "N " + item.itemRate * item.itemsQuantity);
    });

    doc.text(100, 150, "Total:  " + " N " + totalAmount);
    // doc.text(
    //   100,
    //   160,
    //   "CGST+IGST (12%):  " + " &#8358;" + (totalAmount * 12) / 100
    // );
    // doc.text(
    //   100,
    //   170,
